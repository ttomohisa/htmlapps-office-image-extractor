...
foreach ($check in $checks) { if ($check.Failed) { throw $check.Message } }
...
if (-not $restoredHtml.TrimStart().StartsWith("<!doctype html>", [StringComparison]::OrdinalIgnoreCase)) { throw "The restored payload is not an HTML document." }
